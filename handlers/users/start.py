...
from utils.core import get_user, phone_number_validater, send_main_menu, check_all_channels, stoa, send_channels


@dp.message_handler(IsAllowed(), CommandStart(), state='*')
async def bot_start(message: types.Message, key):
...
async def input_phone_number(message: Message, key):
    ...
    if key:
        ...
        if phone_number is not None:
            ...
            await message.answer(
                text="Familiya ismingizni ketma-ket kiriting, masalan:\n\n<b>👉   Alisherov Valisher</b>",
...
async def input_full_name(message: Message, state: FSMContext, key):
    """Get user full name"""
    if key:
        user: User = await get_user(message.from_user)
...
async def input_district(call: types.CallbackQuery, callback_data: dict, key):
    ...
    if key:
        ...
        await send_channels(call.from_user.id)
...
async def check_if_user_allowed(call: types.CallbackQuery):
    ...
    if is_fully_registered:
        ...
        await call.answer(
            "🚫 Kechirasiz botdan to'liq foydalana olmaysiz.")
