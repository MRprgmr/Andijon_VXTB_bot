...
from keyboards.inline.private_templates import get_all_districts, district_callback, get_required_channels_checker, get_school_type, get_schools_list, school_name_callback, school_type_callback
...
async def input_district(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    text, keyboard = await stoa(get_school_type)()
    await call.message.edit_text(text=text, reply_markup=keyboard)
    await RegistrationState.school_type_state.set()
    

@dp.callback_query_handler(IsAllowed(), school_type_callback.filter(), state=RegistrationState.school_type_state)
async def show_schools_list(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    """Ask name of school"""
    
    user: User = await get_user(call.from_user)
    text, keyboard = await stoa(get_schools_list)(callback_data['school_type'], user)
    await call.message.edit_text(text=text, reply_markup=keyboard)
...
async def input_school_name(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    user: User = await get_user(call.from_user)
    user.school = callback_data['name']
    user.is_registered = True
    await stoa(user.save)()
    await call.message.delete()
    await call.message.answer(text="🤖 Tabriklayman, siz botdan muvoffaqiyatli ro'yxatdan o'tdingiz,\n endi undan to'liq foydalana olasiz.")
    await send_main_menu(user)
...
async def check_if_user_allowed(call: types.CallbackQuery, state: FSMContext):
    ...
    if is_fully_registered:
        ...
        await call.answer("🚫 Kechirasiz botdan to'liq foydalana olmaysiz, iltimos quyidagi kanallarning barchasiga a'zo bo'lib so'ng qayta tekshiring")